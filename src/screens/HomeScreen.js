...
const HomeScreen = () => {
  ...
  const [selectedCallEnScreenToSport, setSelectedCallEnScreenToSport] =
    useState('Home');

  const styles = createCallEnStyles(dimensions);
  const [isCreatingWorkoutNow, setIsCreatingWorkoutNow] = useState(false);
  const [isSelectCategoryWasVisible, setIsSelectCategoryWasVisible] =
    useState(false);
  ...
  const saveNewWorkout = async () => {
    try {
      ...
      const newWorkoutIdentify =
        ownedWorkouts.length > 0
          ? Math.max(...ownedWorkouts.map(newWorkout => newWorkout.id)) + 1
          : 1;
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <View
        ...
        {selectedCallEnScreenToSport === 'Home' ? (
                ...
                <>
                  {!isSelectCategoryWasVisible ? (
                    ...
                    <>
                      ...
                      {nameWorkout.length > 0 &&
                        repeatingWorkout.length > 0 &&
                        executionTime.length > 0 &&
                        selectedRestTime.length > 0 && (
                          <TouchableOpacity
                            onPress={() => {
                              saveNewWorkout();
                            }}
          ...
          <CallEnSettingsScreen
            setSelectedCallEnScreenToSport={setSelectedCallEnScreenToSport}
          />
        ) : selectedCallEnScreenToSport === 'SportsFacts' ? (
          <CallEmSportsFactsScreen
            setSelectedCallEnScreenToSport={setSelectedCallEnScreenToSport}
            workoutIcons={workoutIcons}
          />
        ) : selectedCallEnScreenToSport === 'Trainings' ? (
          <CallEnTrainingsScreen
            setSelectedCallEnScreenToSport={setSelectedCallEnScreenToSport}
            setOwnedWorkouts={setOwnedWorkouts}
            ownedWorkouts={ownedWorkouts}
            workoutIcons={workoutIcons}
          />
        ) : null}
